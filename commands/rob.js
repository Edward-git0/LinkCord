...
module.exports = {
    ...
    exec: async (call) => {
        try {
			let cooldown = call.client.memoryData
			console.log(cooldown)
			if(cooldown.has(call.message.author.id)) {
				if(cooldown.get(call.message.author.id).expires < Date.now())
					return call.message.channel.send(`Give it some time, you can't go robbing everyone!`)
				if(Date.now() < cooldown.get(call.message.author.id).expires) {
					cooldown.delete(call.message.author.id)
				}
			}
			...
			if(poss < prob) {
				...
				cooldown.set(call.message.author.id, {
					expires: Date.now() + ms('8m')
				})
