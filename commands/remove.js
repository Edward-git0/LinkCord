module.exports = {
    name: "remove",
    description: "removes money from a specific user",
    category: "economy",
    execute(message, args, Discord, db) {
      ...
      if (!amount || isNaN(amount)) return message.channel.send("Please enter a **valid** amount of money. :x:");
        ...
        db.subtract(`${user.id}.balance`, amount)
        message.channel.send("Successfully removed $" + amount + ` from ${user.username}'s balance! <:linkcoin:670675326837194782>`)
      } else {
